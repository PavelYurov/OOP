...
class MemoryLimit : public Exception {
public:
	MemoryLimit(const char* str = "not enough memory") : Exception(str) {

	}
	//вроде ничегоне нужно больше
};
...
protected:
	...
	int height;
	int width;
	virtual void rebuild(int Height, int Width) {
		if (Height <= 0 || Width <= 0)
		...
		height = Height;
		width = Width;
		if (!(ptr = new T * [height])) throw MemoryLimit();
		for (int i = 0; i < height; i++) {
			if(!(ptr[i] = new T[width])) throw MemoryLimit();
		}
	}
public:
...
public:
	...
	ChildMatrix<T>(const ChildMatrix<T>& ptrM) {
		...
		if(!(this->ptr = new T * [this->height])) throw MemoryLimit();
		for (int i = 0; i < this->height; i++) {
			if(!(this->ptr[i] = new T[this->width])) throw MemoryLimit();
...
std::istream& operator >> (std::istream& ustream, BaseMatrix<T>& obj)
{
	...
	for (int i = 0; i < obj.height; i++) {
		for (int j = 0; j < obj.width; j++) {
			/*T a = NULL;
			ustream >> a;
			if (typeid(a).name() != typeid(obj.ptr[i][j]).name()) throw WrongType();
			obj.ptr[i][j] = a;*/
			ustream >> obj.ptr[i][j];
		}
	}
